...
/* harmony import */ var _modules_showPictureByHover__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./modules/showPictureByHover */ "./src/js/modules/showPictureByHover.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_showPictureByHover__WEBPACK_IMPORTED_MODULE_6__["default"])('.sizes-block');
  Object(_modules_accordion__WEBPACK_IMPORTED_MODULE_7__["default"])('.accordion-heading');
  Object(_modules_menuBurger__WEBPACK_IMPORTED_MODULE_8__["default"])('.burger-menu', '.burger');
...
var accordion = function accordion(triggerSelector) {
  var btnAccordion = document.querySelectorAll(triggerSelector);
  btnAccordion.forEach(function (btns) {
    btns.addEventListener('click', function () {
      var _this = this;

      // Скрыть все элементы
      var blockAccordion = document.querySelectorAll('.accordion-block');
      blockAccordion.forEach(function (block) {
        if (block !== _this.nextElementSibling) {
          // если текущий элемент не является следующим соседним элементом, то выполняется следующее.
      ...
      if (this.classList.contains('active-style')) {
        // если заголовок аккордеона имеет класс active-style, то выполняется следующее.
...
var drop = function drop() {
  ...
  ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(function (eventName) {
    fileInputs.forEach(function (inputs) {
...
var filter = function filter() {
  ...
  var initializeFilter = function initializeFilter(trigger, selector) {
  ...
  initializeFilter('.portfolio-menu .lovers', '.lovers'); // btnAll.addEventListener('click', ()=> {
  //     typeFilter(markAll)
  // });
...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  burgerElement.addEventListener('click', function () {
    if (menuElement.style.display =  true && window.screen.availWidth < 993) {
...
var scrolling = function scrolling(upSelector) {
  ...
  var links = document.querySelectorAll('[href^="#"]'); // Устанавливаем скорость прокрутки

  var speed = 0.3; // Добавляем обработчик событий для каждой ссылки

  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      // Отменяем стандартное поведение ссылки
      event.preventDefault(); // Получаем текущее положение страницы

      var widthTop = document.documentElement.scrollTop; // Переменная widthTop содержит текущее положение страницы относительно верха документа.
      // Получаем хэш ссылки, на которую нажали

      var hash = this.hash; // Получаем координаты блока, к которому нужно прокрутить страницу

      var toBlock = document.querySelector(hash).getBoundingClientRect().top; // Метод getBoundingClientRect() возвращает объект, содержащий значения свойств top, right, bottom, left, width, и height элемента.
      // Устанавливаем время начала прокрутки

      var start = null; // Функция, которая выполняется на каждом кадре анимации
      ...
      function step(time) {
        ...
        var r = toBlock < 0 ? Math.max(widthTop - progress / speed, widthTop + toBlock) : Math.min(widthTop + progress / speed, widthTop + toBlock); // мы используем тернарный оператор для определения новой позиции страницы. Если toBlock (верхняя координата блока) меньше нуля,
        ...
        document.documentElement.scrollTo(0, r); // Проверяем, достигли ли мы нужного положения
...
/***/ "./src/js/modules/showPictureByHover.js":
/*!**********************************************!*\
  !*** ./src/js/modules/showPictureByHover.js ***!
  \**********************************************/
/*! exports provided: default */
...
var showPictureByHover = function showPictureByHover(imgSelector) {
...
/* harmony default export */ __webpack_exports__["default"] = (showPictureByHover);
